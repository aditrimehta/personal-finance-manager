...
export default function Profile() {
  ...
  const handleSaveLimits = async () => {
    ...
    try {
      ...
      const entries = Object.entries(limitDraft).filter(
        ([, v]) => v !== "" && v !== null,
      );
      ...
      const merged = {
        ...catLimits,
        ...Object.fromEntries(entries.map(([k, v]) => [k, Number(v)])),
      };
      setCatLimits(merged);
  ...
  const savingsPct = user.savingTarget
    ? Math.round((user.savedSoFar / user.savingTarget) * 100)
    : 0;
  const spentPct = user.spendingLimit
    ? Math.round((user.totalSpent / user.spendingLimit) * 100)
    : 0;
  ...
  return (
    <AppLayout>
      <div className="profile-page">
        <div className="dash-topbar">
          ...
          {!editing ? (
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button className="cancel-btn" onClick={() => { setEditing(false); setEditingLimits(false); setLimitDraft({}); }}>Cancel</button>
        ...
        {saved && (
          <div className="save-toast fade-up">✓ Saved successfully</div>
        )}
          ...
          <div className="profile-stats fade-up-1">
            {[
              ...
              },
              {
                ...
                val: `₹${user.spendingLimit.toLocaleString()}`,
                icon: "🎯",
                color: "amber",
              },
              {
                label: "Savings Goal",
                val: `₹${user.savingTarget.toLocaleString()}`,
                icon: "🏦",
                color: "green",
              },
              {
                label: "Saved So Far",
                val: `₹${user.savedSoFar.toLocaleString()}`,
                icon: "📈",
                color: "purple",
              },
            ].map((s) => (
            ...
            <div className="detail-grid">
              <DetailField
                ...
                input={
                  <select
                    className="form-input"
                    value={draft.monthlyIncome ? "Monthly" : "Annual"}
                    onChange={(e) => {
                      ...
                      setDraft((d) => ({
                        ...
                        monthlyIncome:
                          v === "Monthly"
                            ? d.monthlyIncome || d.annualIncome
                            : null,
                        annualIncome:
                          v === "Annual"
                            ? d.annualIncome || d.monthlyIncome
                            : null,
                      }));
                    }}
                  >
              ...
              <DetailField
                ...
                value={`₹${user.spendingLimit.toLocaleString()}`}
                editing={editing}
                input={
                  <input
                    className="form-input"
                    ...
                    onChange={upd("spendingLimit")}
                  />
                }
              />
              <DetailField
                label="Saving Target"
                value={`₹${user.savingTarget.toLocaleString()}`}
                editing={editing}
                input={
                  <input
                    className="form-input"
                    type="number"
                    value={draft.savingTarget}
                    onChange={upd("savingTarget")}
                  />
                }
              />
            </div>
          ...
          <div className="card fade-up-3">
            ...
            <div className="progress-items">
              ...
              <ProgressItem
                ...
                pct={spentPct}
                color={spentPct > 80 ? "#f87171" : "#4f8ef7"}
              />
            </div>
          </div>
...
<div className="card fade-up-4">
  ...
  <div className="cat-chips-prof">
    {categories.map((c) => (
      <span key={c.id} className="cat-chip-prof">{c.name}</span>
    ))}
    {editing && (
      <button
        type="button"
                  ...
                  <div key={c.name} className="cat-limit-row">
                    ...
                    {editingLimits ? (
                      <input
                        ...
                        min="0"
                        placeholder={
                          current ? `₹${current.toLocaleString()}` : "No limit"
                        }
  ...
  return (
    <div className="prog-item">
      <div className="prog-top">
        ...
        <span className="prog-pct" style={{ color }}>
          {pct}%
        </span>
      </div>
