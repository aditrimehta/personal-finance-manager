  ...
  const handleFinish = async () => {
    ...
    try {
      const res = await fetch(
        "http://localhost:8000/api/preferences/onboarding/",
        {
          method: "POST",
          headers: {
        ...
        <div className="ob-panel" key={step}>
                ...
                {[
                  {
                    id: "personal",
                    label: "Personal",
                    desc: "Track my own income & spending",
                    icon: "👤",
                  },
                  {
                    id: "family",
                    label: "Family",
                    desc: "Manage household budget together",
                    icon: "🏠",
                  },
                  {
                    id: "freelance",
                    label: "Freelancer",
                    desc: "Client income, project expenses, taxes",
                    icon: "💼",
                  },
                  {
                    id: "business",
                    label: "Small Business",
                    desc: "Monitor business cash flow & limits",
                    icon: "🏢",
                  },
                ].map((opt) => (
                  <button
          ...
          {step === 1 && (
            <div className="ob-step-content">
              ...
              <p className="ob-desc">
                Help us calibrate your spending & savings overview.
              </p>
              ...
              <div className="ob-fields">
                ...
                {data.savingTarget && data.income && (
                  <div className="ob-insight">
                    ...
                    <span>
                      Saving 20% of your income means reaching your goal in ~
                      {Math.ceil(data.savingTarget / (data.income * 0.2))}{" "}
                      months.
                    </span>
                  </div>
                )}
          ...
          {step === 3 && (
            <div className="ob-step-content">
              ...
              <p className="ob-desc">
                Select the ones you spend in. You can add limits later.
              </p>
          ...
          {step === 4 && (
            <div className="ob-step-content">
              ...
              <p className="ob-desc">
                Here's a summary of your profile. You can edit these any time.
              </p>
              <div className="ob-summary">
                {[
                  { label: "Purpose", value: data.purpose || "—" },
                  {
                    label: "Income",
                    value: data.income
                      ? `₹${Number(data.income).toLocaleString()} / ${data.incomeType}`
                      : "—",
                  },
                  {
                    label: "Spending limit",
                    value: data.spendingLimit
                      ? `₹${Number(data.spendingLimit).toLocaleString()} / month`
                      : "—",
                  },
                  { label: "Saving goal", value: data.savingGoal || "—" },
                  {
                    label: "Target amount",
                    value: data.savingTarget
                      ? `₹${Number(data.savingTarget).toLocaleString()}`
                      : "—",
                  },
                  {
                    label: "Categories",
                    value: `${data.categories.length} selected`,
                  },
                ].map((r) => (
              ...
              <div className="ob-cat-tags">
                {data.categories.map((id) => {
                  ...
                  return (
                    <span key={id} className="ob-cat-tag">
                      {c?.icon} {c?.label}
                    </span>
                  );
                })}
